"""Conditional-antithetic SliceEqOcc on the locked PROMISE12 recipe.

This independent successor preserves SliceEqOcc and changes only the
unlabeled acquisition-risk estimator.  For each unlabeled anatomy it evaluates
the sampled profile and its same-width, phase-reflected partner, then averages
their fractional-occupancy losses without changing the total unlabeled weight.
"""
...
parser.add_argument('--exp', type=str, default='SliceEqOccCAP_PROMISE12',
...
def _validate_args(flags):
    ...
    if flags.batch_size != 24 or flags.labeled_bs != 12:
        raise ValueError(
            'SliceEqOccCAP expects loader batch_size=24 and labeled_bs=12; '
            'its fixed post-warmup student batch is 48')
...
def self_train(flags, pretrained_checkpoint, snapshot_path):
    ...
    logging.info(
        'SliceEqOccCAP effective student batch after warmup: 48 '
        '(12 original-L + 12 reacquired-L + 12 primary-U + '
        '12 phase-reflected-U)')
    logging.info(
        'SliceEqOccCAP objective: Lsup=0.5*(L_original_hard + '
        'L_reacquired_labeled_soft); U=0.5*(U_primary_soft + '
        'U_phase_reflected_soft); total=Lsup+lambda*U')
    logging.info(
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num < 1000:
                ...
                unlabeled_antithetic_images = unlabeled_images
                unlabeled_antithetic_occupancy = unlabeled_occupancy
                primary_consistency_loss = zero
                primary_consistency_ce = zero
                primary_consistency_dice = zero
                antithetic_consistency_loss = zero
                antithetic_consistency_ce = zero
                antithetic_consistency_dice = zero
                antithetic_diagnostics = {
                    'phase_pair_residual': zero,
                    'weight_l1_separation': zero,
                    'center_weight_gap': zero,
                    'image_absolute_separation': zero,
                    'occupancy_absolute_separation': zero,
                    'hard_target_disagreement': zero,
                }
                diagnostics = {
                ...
                with torch.no_grad():
                    ...
                    unlabeled_antithetic_weights = \
                        antithetic_phase_weights(
                            offsets, unlabeled_sigma, unlabeled_phase)
                    ...
                    unlabeled_antithetic_images, \
                        unlabeled_antithetic_hard_target, \
                        unlabeled_antithetic_occupancy = \
                        paired_slice_reacquisition(
                            unlabeled_stack, pseudo_stack,
                            unlabeled_antithetic_weights, num_classes)
                    antithetic_diagnostics = antithetic_pair_diagnostics(
                        unlabeled_weights, unlabeled_antithetic_weights,
                ...
                student_batch = torch.cat(
                    (labeled_images, labeled_reacquired_images,
                     unlabeled_reacquired_images,
                     unlabeled_antithetic_images), dim=0)
                ...
                primary_unlabeled_outputs = outputs[
                    2 * flags.labeled_bs:3 * flags.labeled_bs]
                antithetic_unlabeled_outputs = outputs[
                    3 * flags.labeled_bs:]
                ...
                primary_consistency_loss, primary_consistency_ce, \
                    primary_consistency_dice = soft_segmentation_loss(
                        primary_unlabeled_outputs, unlabeled_occupancy)
                antithetic_consistency_loss, antithetic_consistency_ce, \
                    antithetic_consistency_dice = soft_segmentation_loss(
                        antithetic_unlabeled_outputs,
                        unlabeled_antithetic_occupancy)
                consistency_loss = 0.5 * (
                    primary_consistency_loss +
                    antithetic_consistency_loss)
                consistency_ce = 0.5 * (
                    primary_consistency_ce + antithetic_consistency_ce)
                consistency_dice = 0.5 * (
                    primary_consistency_dice +
                    antithetic_consistency_dice)
                loss = supervised_loss + \
                    consistency_weight * consistency_loss

            if not torch.isfinite(loss):
                raise FloatingPointError(
                    'Non-finite SliceEqOccCAP loss at iteration {}'.format(
                        iter_num))
            loss.backward()
            optimizer.step()
            ...
            writer.add_scalar('info/primary_consistency_loss',
                              primary_consistency_loss, iter_num)
            writer.add_scalar('info/antithetic_consistency_loss',
                              antithetic_consistency_loss, iter_num)
            for name, value in antithetic_diagnostics.items():
                if not torch.isfinite(value):
                    raise FloatingPointError(
                        'Non-finite SliceEqOccCAP diagnostic {}'.format(name))
                writer.add_scalar(
                    'sliceeq_occ_cap/' + name, value, iter_num)
            ...
            if iter_num % 20 == 0:
                ...
                writer.add_image(
                    'sliceeq_occ_cap/AntitheticUnlabeled',
                    unlabeled_antithetic_images[0], iter_num)
                writer.add_image(
                    'sliceeq_occ_cap/AntitheticForegroundOccupancy',
                    unlabeled_antithetic_occupancy[0, 1:2], iter_num)
                writer.add_image(
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                logging.info(
                    'SliceEqOccCAP train iteration %d: lambda=%.6f '
                    'loss(original/L-eq/U-mean)=%.6f/%.6f/%.6f; '
                ...
                logging.info(
                    'SliceEqOccCAP antithetic iteration %d: '
                    'loss(primary/reflected)=%.6f/%.6f; '
                    'pair(weight_l1/image/occupancy/hard_disagree/'
                    'phase_residual)=%.6f/%.6f/%.6f/%.6f/%.8f',
                    iter_num,
                    primary_consistency_loss.item(),
                    antithetic_consistency_loss.item(),
                    antithetic_diagnostics[
                        'weight_l1_separation'].item(),
                    antithetic_diagnostics[
                        'image_absolute_separation'].item(),
                    antithetic_diagnostics[
                        'occupancy_absolute_separation'].item(),
                    antithetic_diagnostics[
                        'hard_target_disagreement'].item(),
                    antithetic_diagnostics[
                        'phase_pair_residual'].item())
                logging.info(
...
if __name__ == '__main__':
    ...
    logging.info(
        '================ START SliceEqOccCAP SELF-TRAINING ================')
