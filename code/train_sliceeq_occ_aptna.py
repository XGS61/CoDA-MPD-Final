"""Acquisition-preserving transient native anchor for SliceEqOcc.

This independent successor keeps the complete 36-view SliceEqOcc main path and
its full unlabeled fractional-occupancy coefficient. A native-U hard
pseudo-label anchor is added only as a transient auxiliary objective. Its
forward uses frozen student BatchNorm running statistics and an isolated CUDA
RNG stream, while the EMA teacher deliberately remains in train mode.
"""
...
parser.add_argument('--exp', type=str, default='SliceEqOccAPTNA_PROMISE12',
...
def _validate_args(flags):
    ...
    if flags.batch_size != 24 or flags.labeled_bs != 12:
        raise ValueError(
            'SliceEqOccAPTNA expects loader batch_size=24 and labeled_bs=12; '
            'its main student batch remains the locked SliceEqOcc batch 36')
...
def _transient_native_weight(consistency_weight, consistency_max):
    """Derive AP-TNA weight from the inherited ramp without new parameters."""
    if consistency_max <= 0:
        raise ValueError('consistency must be positive for AP-TNA')
    weight = float(consistency_weight)
    progress = min(max(weight / float(consistency_max), 0.0), 1.0)
    native_weight = 0.5 * weight * (1.0 - progress)
    if not 0.0 <= native_weight <= 0.5 * weight + 1e-12:
        raise AssertionError('AP-TNA native weight left its locked bounds')
    return native_weight


@contextmanager
def _frozen_batch_norm_running_stats(network):
    """Use BN running statistics for the auxiliary forward only."""
    batch_norms = [
...
def _native_auxiliary_logits(network, images, seed):
    """Run native-U without changing parent BN buffers or global CUDA RNG."""
    device_index = images.device.index
    devices = [] if device_index is None else [device_index]
    with torch.random.fork_rng(devices=devices):
        if device_index is not None:
            torch.cuda.manual_seed(seed)
        with _frozen_batch_norm_running_stats(network):
            return _logits(network(images))
...
def self_train(flags, pretrained_checkpoint, snapshot_path):
    ...
    consistency_max = 5.0 * flags.consistency
    ...
    logging.info(
        'SliceEqOccAPTNA main student batch after warmup: 36 '
        '(12 original-L + 12 reacquired-L + 12 reacquired-U); '
        'native-U uses a separate BN-isolated 12-view auxiliary forward')
    logging.info(
        'SliceEqOccAPTNA objective: Lsup=0.5*(L_original_hard + '
        'L_reacquired_labeled_soft); total=Lsup+lambda*U_measurement_soft'
        '+mu*U_native_hard, mu=0.5*lambda*(1-lambda/lambda_max)')
    logging.info(
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            consistency_weight = base.get_current_consistency_weight(
                iter_num // 150)
            native_consistency_weight = 0.0 if iter_num < 1000 else \
                _transient_native_weight(
                    consistency_weight, consistency_max)
            ...
            if iter_num < 1000:
                ...
                measurement_consistency_loss = consistency_loss
                measurement_consistency_ce = consistency_ce
                measurement_consistency_dice = consistency_dice
                native_consistency_loss = volume_batch.new_tensor(0.0)
                native_consistency_ce = volume_batch.new_tensor(0.0)
                native_consistency_dice = volume_batch.new_tensor(0.0)
                ...
                measurement_consistency_loss, \
                    measurement_consistency_ce, \
                    measurement_consistency_dice = soft_segmentation_loss(
                        unlabeled_outputs, unlabeled_occupancy)
                consistency_loss = measurement_consistency_loss
                consistency_ce = measurement_consistency_ce
                consistency_dice = measurement_consistency_dice

                native_unlabeled_target = pseudo_stack[:, center]
                auxiliary_seed = flags.seed + 1000003 + iter_num
                native_unlabeled_outputs = _native_auxiliary_logits(
                    model, unlabeled_images, auxiliary_seed)
                native_consistency_loss, native_consistency_ce, \
                    native_consistency_dice = _hard_segmentation_losses(
                        native_unlabeled_outputs, native_unlabeled_target,
                        ce_loss, dice_loss)

                loss = supervised_loss + \
                    consistency_weight * measurement_consistency_loss + \
                    native_consistency_weight * native_consistency_loss

            if not torch.isfinite(loss):
                raise FloatingPointError(
                    'Non-finite SliceEqOccAPTNA loss at iteration {}'.format(
                        iter_num))
            loss.backward()
            optimizer.step()
            ...
            writer.add_scalar('info/measurement_consistency_loss',
                              measurement_consistency_loss, iter_num)
            writer.add_scalar('info/native_consistency_loss',
                              native_consistency_loss, iter_num)
            writer.add_scalar('info/native_consistency_ce',
                              native_consistency_ce, iter_num)
            writer.add_scalar('info/native_consistency_dice',
                              native_consistency_dice, iter_num)
            writer.add_scalar('info/native_consistency_weight',
                              native_consistency_weight, iter_num)
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                logging.info(
                    'SliceEqOccAPTNA transient anchor iteration %d: '
                    'weight(measurement/native)=%.6f/%.6f; '
                    'loss(measurement/native)=%.6f/%.6f',
                    iter_num,
                    float(consistency_weight),
                    float(native_consistency_weight),
                    measurement_consistency_loss.item(),
                    native_consistency_loss.item())
                logging.info(
...
if __name__ == '__main__':
    ...
    logging.info(
        '============== START SliceEqOccAPTNA SELF-TRAINING ==============')
