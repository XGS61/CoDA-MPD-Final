...
from utils.coda import (couple_pseudo_target, evidence_augment,
                        lcc_preserve_teacher_probabilities,
                        soft_cross_entropy, soft_dice_loss)
...
parser.add_argument('--root_path', type=str,
                    default='/home/aiteam/zhengtaoma/Baseline/data/PROMISE12_h5_training_source', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='CoDA_MT_PROMISE12', help='experiment_name')
...
parser.add_argument('--coda_augmentations', type=str,
                    default='resolution,gaussian_noise',
                    help='comma-separated CoDA strong-view families')
parser.add_argument('--coda_resolution_min', type=float, default=0.25,
                    help='minimum downsample scale for CoDA resolution degradation')
parser.add_argument('--coda_resolution_max', type=float, default=0.75,
                    help='maximum downsample scale for CoDA resolution degradation')
parser.add_argument('--coda_noise_min', type=float, default=0.05,
                    help='minimum noise std relative to each slice std')
parser.add_argument('--coda_noise_max', type=float, default=0.20,
                    help='maximum noise std relative to each slice std')
parser.add_argument('--coda_evidence_kernel', type=int, default=7,
                    help='odd local window for realized evidence loss')
...
def load_pretrained_checkpoint(model, ema_model, optimizer, path):
    """Strictly restore the shared baseline network and optimizer state."""
    checkpoint = torch.load(path, map_location='cuda')
    if not isinstance(checkpoint, dict) or 'net' not in checkpoint:
        raise RuntimeError(
            'Shared pretrain checkpoint must contain `net`: {}'.format(path))
    if 'opt' not in checkpoint:
        raise RuntimeError(
            'Shared pretrain checkpoint must contain `opt`: {}'.format(path))
    model.load_state_dict(checkpoint['net'], strict=True)
    ema_model.load_state_dict(checkpoint['net'], strict=True)
    optimizer.load_state_dict(checkpoint['opt'])


def reset_stage_rng(seed):
    random.seed(seed)
    np.random.seed(seed)
...
def seed_data_worker(worker_id, base_seed):
    """Seed a DataLoader worker with a Windows-spawn-safe callback."""
...
def patients_to_slices(dataset, patiens_num):
    ...
    elif "Prostate" in dataset or "PROMISE12" in dataset:
...
def pre_train(args, snapshot_path):
    ...
    trainloader = DataLoader(db_train, batch_sampler=batch_sampler,
                             ...
                             worker_init_fn=partial(seed_data_worker,
                                                    base_seed=args.seed))
...
def self_train(args, pretrained_checkpoint, snapshot_path):
    base_lr = args.base_lr
    num_classes = args.num_classes
    batch_size = args.batch_size
    ...
    logging.info('Shared pretrain SHA-256: %s',
                 checkpoint_sha256(pretrained_checkpoint))
    reset_stage_rng(args.seed)
    ...
    # Use an independent, reproducible stream so CoDA sampling does not alter
    # the Baseline's model/dropout random-number sequence.
    coda_generator = torch.Generator(device='cuda')
    coda_generator.manual_seed(args.seed)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # The loader output is the weak teacher view. Keep the Baseline's
            # first 1,000 self-training iterations unchanged, including its
            # student input, because the consistency loss is zero there.
            ema_inputs = unlabeled_volume_batch
            if iter_num < 1000:
                strong_unlabeled = unlabeled_volume_batch
                evidence_loss = torch.zeros_like(unlabeled_volume_batch[:, :1])
                coda_metadata = {
                    'family': 'warmup_identity',
                    'family_id': evidence_loss.new_tensor(-1.0),
                    'sampled_family_id': evidence_loss.new_tensor(-1.0),
                    'severity': evidence_loss.new_tensor(0.0),
                }
            else:
                strong_unlabeled, evidence_loss, coda_metadata = evidence_augment(
                    unlabeled_volume_batch,
                    augmentations=args.coda_augmentations,
                    resolution_scale=(args.coda_resolution_min,
                                      args.coda_resolution_max),
                    noise_std=(args.coda_noise_min, args.coda_noise_max),
                    evidence_kernel=args.coda_evidence_kernel,
                    generator=coda_generator)

            student_volume_batch = torch.cat(
                (volume_batch[:args.labeled_bs], strong_unlabeled), dim=0)
            ...
            with torch.no_grad():
                ...
                ema_output_soft = lcc_preserve_teacher_probabilities(
                    ema_output, pseudo_labels)
                coda_target = couple_pseudo_target(
                    ema_output_soft, evidence_loss)
            ...
            if iter_num < 1000:
                ...
                unl_ce = soft_cross_entropy(unl_outputs, coda_target)
                unl_dice = soft_dice_loss(unl_outputs_soft, coda_target)
            ...
            writer.add_scalar('coda/evidence_loss_mean',
                              evidence_loss.mean(), iter_num)
            writer.add_scalar('coda/augmentation_family_id',
                              coda_metadata['family_id'], iter_num)
            writer.add_scalar('coda/sampled_family_id',
                              coda_metadata['sampled_family_id'], iter_num)
            writer.add_scalar('coda/severity',
                              coda_metadata['severity'], iter_num)
            target_entropy = -(coda_target * torch.log(
                coda_target.clamp_min(1e-7))).sum(dim=1).mean()
            writer.add_scalar('coda/target_entropy', target_entropy, iter_num)
            ...
            if iter_num % 20 == 0:
                ...
                writer.add_image('coda/StrongUnlabeled',
                                 strong_unlabeled[0], iter_num)
                writer.add_image('coda/EvidenceLoss',
                                 evidence_loss[0], iter_num)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        force=True)
    logging.info("================ START CoDA SELF-TRAINING ================")
