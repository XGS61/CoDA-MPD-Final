"""Acquisition-aligned dropout reliability for the SliceEqOcc recipe.

This independent successor leaves the complete SliceEqOcc objective intact.
After the inherited 1k warmup, two isolated stochastic EMA measurements are
projected by one shared slice profile. Their mean occupancy is supervised with
continuous operator-space JS reliability; validation and 2D inference remain
unchanged.
"""
...
from utils.sliceeq_adu import (
    acquisition_aligned_reliability, adu_diagnostics,
...
parser.add_argument('--exp', type=str, default='SliceEqOccADU_PROMISE12',
...
def _validate_args(flags):
    ...
    for name, expected in locked_recipe.items():
        ...
        if actual != expected:
            raise ValueError(
                'H7.11 locks --{}={!r}; received {!r}'.format(
                    name, expected, actual))


def _validate_pretrained_checkpoint_hash(path):
    actual = locked._sha256(path)
    if actual != EXPECTED_PRETRAINED_SHA256:
        raise RuntimeError(
            'H7.11 requires shared Pre10000 checkpoint SHA-256 {}; got {} '
            'for {}'.format(EXPECTED_PRETRAINED_SHA256, actual, path))
    return actual
...
ADU_SECOND_TEACHER_SEED_OFFSET = 7000003


def _isolated_second_teacher_logits(ema_model, teacher_inputs, seed):
    """Run one independent train-mode EMA pass with zero persistent state."""
    return _logits(isolated_stochastic_teacher_forward(
        ema_model, teacher_inputs, seed))
...
def self_train(flags, pretrained_checkpoint, snapshot_path):
    ...
    logging.info(
        'SliceEqOccADU objective: parent Lsup is unchanged; '
        'total=Lsup+lambda*L_ADU_weighted_unlabeled_occupancy')
    logging.info(
        'SliceEqOccADU profile RNG seeds: unlabeled=%d, labeled=%d; '
        'isolated second-teacher seed offset=%d',
        flags.seed, flags.seed + 1, ADU_SECOND_TEACHER_SEED_OFFSET)
        ...
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num < 1000:
                ...
                adu_activity = {
                    'js_mean': zero,
                    'js_max': zero,
                    'positive_js_fraction': zero,
                    'reliability_mean': volume_batch.new_tensor(1.0),
                    'effective_sample_fraction': volume_batch.new_tensor(1.0),
                    'pseudo_fractional_support_mean_weight':
                        volume_batch.new_tensor(1.0),
                    'occupancy_absolute_disagreement': zero,
                    'occupancy_hard_disagreement': zero,
                    'mean_target_absolute_change_from_primary': zero,
                }
            else:
                with torch.no_grad():
                    ...
                    teacher_inputs = unlabeled_stack.reshape(
                        ...
                        height, width)
                    primary_ema_output = _logits(
                        ema_model(teacher_inputs))
                    primary_pseudo_stack = base.get_masks(
                        primary_ema_output, nms=1).reshape(
                            unlabeled_count, stack_size, height, width)
                    secondary_ema_output = _isolated_second_teacher_logits(
                        ema_model, teacher_inputs,
                        flags.seed + ADU_SECOND_TEACHER_SEED_OFFSET +
                        iter_num)
                    secondary_pseudo_stack = base.get_masks(
                        secondary_ema_output, nms=1).reshape(
                            unlabeled_count, stack_size, height, width)
                    del primary_ema_output, secondary_ema_output
                    pseudo_stack = primary_pseudo_stack
                    ...
                    unlabeled_reacquired_images, _, \
                        primary_unlabeled_occupancy = \
                        paired_slice_reacquisition(
                            unlabeled_stack, primary_pseudo_stack,
                            unlabeled_weights, num_classes)
                    secondary_reacquired_images, _, \
                        secondary_unlabeled_occupancy = \
                        paired_slice_reacquisition(
                            unlabeled_stack, secondary_pseudo_stack,
                            unlabeled_weights, num_classes)
                    if not torch.equal(
                            unlabeled_reacquired_images,
                            secondary_reacquired_images):
                        raise RuntimeError(
                            'ADU paired profiles produced different images')
                    unlabeled_occupancy, adu_js, adu_reliability = \
                        acquisition_aligned_reliability(
                            primary_unlabeled_occupancy,
                            secondary_unlabeled_occupancy)
                    unlabeled_hard_target = unlabeled_occupancy.argmax(dim=1)
                    adu_activity = adu_diagnostics(
                        primary_unlabeled_occupancy,
                        secondary_unlabeled_occupancy,
                        unlabeled_occupancy, adu_js, adu_reliability)
                    del secondary_reacquired_images
                    del primary_unlabeled_occupancy
                    del secondary_unlabeled_occupancy, adu_js
                    del secondary_pseudo_stack, primary_pseudo_stack
            ...
            for name, value in diagnostics.items():
                if not torch.isfinite(value):
                    raise FloatingPointError(
                        'Non-finite SliceEqOccADU diagnostic {}'.format(name))
                writer.add_scalar('sliceeq_occ/' + name, value, iter_num)
            for name, value in adu_activity.items():
                if not torch.isfinite(value):
                    raise FloatingPointError(
                        'Non-finite ADU diagnostic {}'.format(name))
                writer.add_scalar('sliceeq_adu/' + name, value, iter_num)
            writer.add_scalar(
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                logging.info(
                    'SliceEqOccADU occupancy iteration %d: '
                ...
                logging.info(
                    'SliceEqOccADU reliability iteration %d: '
                    'JS(mean/max/active)=%.6f/%.6f/%.6f; '
                    'weight(mean/ESS/pseudo_fractional)=%.6f/%.6f/%.6f; '
                    'occupancy(abs/hard)=%.6f/%.6f',
                    iter_num,
                    adu_activity['js_mean'].item(),
                    adu_activity['js_max'].item(),
                    adu_activity['positive_js_fraction'].item(),
                    adu_activity['reliability_mean'].item(),
                    adu_activity['effective_sample_fraction'].item(),
                    adu_activity[
                        'pseudo_fractional_support_mean_weight'].item(),
                    adu_activity['occupancy_absolute_disagreement'].item(),
                    adu_activity['occupancy_hard_disagreement'].item())
...
if __name__ == '__main__':
    ...
    logging.info(
        '=============== START SliceEqOccADU SELF-TRAINING ==============')
