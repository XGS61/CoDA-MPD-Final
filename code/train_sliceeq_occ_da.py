"""Native-measurement dual-anchor SliceEqOcc on the locked PROMISE12 recipe.

This independent successor preserves SliceEqOcc and adds the missing native
unlabeled semantic anchor. After warmup, each unlabeled anatomy contributes
one native center-slice hard pseudo-label loss and one paired re-acquisition
fractional-occupancy loss. Their mean keeps the total unlabeled weight fixed.
The EMA teacher deliberately remains in train mode to match the baseline and
all preceding SliceEq variants.
"""
...
parser.add_argument('--exp', type=str, default='SliceEqOccDA_PROMISE12',
...
def _validate_args(flags):
    ...
    if flags.batch_size != 24 or flags.labeled_bs != 12:
        raise ValueError(
            'SliceEqOccDA expects loader batch_size=24 and labeled_bs=12; '
...
def self_train(flags, pretrained_checkpoint, snapshot_path):
    ...
    logging.info(
        'SliceEqOccDA effective student batch after warmup: 48 '
        '(12 original-L + 12 reacquired-L + 12 reacquired-U + '
        '12 native-U)')
    logging.info(
        'SliceEqOccDA objective: Lsup=0.5*(L_original_hard + '
        'L_reacquired_labeled_soft); U=0.5*(U_measurement_soft + '
        'U_native_hard); total=Lsup+lambda*U')
    logging.info(
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num < 1000:
                ...
                native_unlabeled_images = unlabeled_images
                native_unlabeled_target = center_pseudo
                measurement_consistency_loss = zero
                measurement_consistency_ce = zero
                measurement_consistency_dice = zero
                native_consistency_loss = zero
                native_consistency_ce = zero
                native_consistency_dice = zero
                diagnostics = {
                ...
                measurement_unlabeled_outputs = outputs[
                    2 * flags.labeled_bs:3 * flags.labeled_bs]
                native_unlabeled_outputs = outputs[
                    3 * flags.labeled_bs:]
                ...
                measurement_consistency_loss, measurement_consistency_ce, \
                    measurement_consistency_dice = soft_segmentation_loss(
                        measurement_unlabeled_outputs, unlabeled_occupancy)
                ...
                consistency_ce = 0.5 * (
                    measurement_consistency_ce + native_consistency_ce)
                consistency_dice = 0.5 * (
                    measurement_consistency_dice +
                    native_consistency_dice)
                loss = supervised_loss + \
                    consistency_weight * consistency_loss

            if not torch.isfinite(loss):
                raise FloatingPointError(
                    'Non-finite SliceEqOccDA loss at iteration {}'.format(
                        iter_num))
            loss.backward()
            optimizer.step()
            ...
            if iter_num % 20 == 0:
                ...
                writer.add_image(
                    'sliceeq_occ_da/NativeUnlabeled',
                    native_unlabeled_images[0], iter_num)
                writer.add_image(
                    'sliceeq_occ_da/NativeTeacherMask',
                    native_unlabeled_target[0].unsqueeze(0) * 50, iter_num)
                writer.add_image(
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                logging.info(
                    'SliceEqOccDA train iteration %d: lambda=%.6f '
                ...
                logging.info(
                    'SliceEqOccDA dual-anchor iteration %d: '
                    'loss(measurement/native)=%.6f/%.6f; '
                    'ce=%.6f/%.6f; dice=%.6f/%.6f',
                    iter_num,
                    measurement_consistency_loss.item(),
                    native_consistency_loss.item(),
                    measurement_consistency_ce.item(),
                    native_consistency_ce.item(),
                    measurement_consistency_dice.item(),
                    native_consistency_dice.item())
                logging.info(
...
if __name__ == '__main__':
    ...
    logging.info(
        '================ START SliceEqOccDA SELF-TRAINING ================')
