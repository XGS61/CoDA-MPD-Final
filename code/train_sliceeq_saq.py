"""Stratified acquisition-quadrature SliceEq on the locked recipe.

This is an independent successor to ``train_sliceeq_occ.py``. It preserves
the full SliceEqOcc objective and changes only profile sampling: the four 2x2
Gauss-Legendre nodes are each assigned three times within every 12-sample
labeled or unlabeled branch using independent seeded permutations.
"""
...
from utils.sliceeq_saq import (
    gauss_legendre_profile_nodes, quadrature_assignment_diagnostics,
...
parser.add_argument('--exp', type=str, default='SliceEqSAQ_PROMISE12',
...
def _validate_args(flags):
    ...
    if flags.batch_size != 24 or flags.labeled_bs != 12:
        raise ValueError(
            'SliceEqSAQ final recipe expects effective student batch 36; '
...
def self_train(flags, pretrained_checkpoint, snapshot_path):
    ...
    quadrature_nodes = gauss_legendre_profile_nodes(
        (flags.sliceeq_sigma_min, flags.sliceeq_sigma_max),
        (flags.sliceeq_phase_min, flags.sliceeq_phase_max))
    logging.info(
        'SliceEqSAQ profile domain: offsets=(-1,0,1), sigma=[%.3f, %.3f], '
        'phase=[%.3f, %.3f]',
        flags.sliceeq_sigma_min, flags.sliceeq_sigma_max,
        flags.sliceeq_phase_min, flags.sliceeq_phase_max)
    logging.info(
        'SliceEqSAQ quadrature nodes (sigma,phase): %s',
        ', '.join('({:.6f},{:.6f})'.format(*node)
                  for node in quadrature_nodes))
    logging.info(
        'SliceEqSAQ node allocation: four nodes x three samples in each '
        '12-sample branch; one seeded permutation per branch and step')
    logging.info(
        'SliceEqSAQ effective student batch after warmup: 36 '
        '(12 original-L + 12 reacquired-L + 12 reacquired-U)')
    logging.info(
        'SliceEqSAQ objective: Lsup=0.5*(L_original_hard + '
        'L_reacquired_labeled_soft); total=Lsup+lambda*L_unlabeled_soft')
    logging.info(
        'SliceEqSAQ permutation RNG seeds: unlabeled=%d, labeled=%d',
        flags.seed, flags.seed + 1)

    # Keep L/U node-to-sample assignments independent.  Profile values are
    # fixed quadrature nodes; only their balanced assignment is randomized.
        ...
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num < 1000:
                ...
                diagnostics = {
                    ...
                    'labeled_center_foreground_fraction': (
                        labeled_labels > 0).float().mean(),
                    'labeled_quadrature_node_coverage': zero,
                    'labeled_quadrature_max_count_deviation': zero,
                    ...
                    'unlabeled_center_foreground_fraction': (
                        center_pseudo > 0).float().mean(),
                    'unlabeled_quadrature_node_coverage': zero,
                    'unlabeled_quadrature_max_count_deviation': zero,
                }
            else:
                with torch.no_grad():
                    ...
                    diagnostics.update(_prefixed(
                        'unlabeled_', occupancy_diagnostics(
                            ...
                            pseudo_stack[:, center])))
                    diagnostics.update(_prefixed(
                        'labeled_', quadrature_assignment_diagnostics(
                            labeled_node_ids)))
                    diagnostics.update(_prefixed(
                        'unlabeled_', quadrature_assignment_diagnostics(
                            unlabeled_node_ids)))
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError(
                    'Non-finite SliceEqSAQ loss at iteration {}'.format(
                        iter_num))
            loss.backward()
            optimizer.step()
            ...
            for name, value in diagnostics.items():
                if not torch.isfinite(value):
                    raise FloatingPointError(
                        'Non-finite SliceEqSAQ diagnostic {}'.format(name))
                writer.add_scalar('sliceeq_saq/' + name, value, iter_num)
            writer.add_scalar(
                'sliceeq_saq/labeled_neighbor_clamped_sample_fraction',
                labeled_clamped.mean(), iter_num)
            writer.add_scalar(
                'sliceeq_saq/unlabeled_neighbor_clamped_sample_fraction',
            ...
            if iter_num % 20 == 0:
                ...
                writer.add_image(
                    'sliceeq_saq/ReacquiredLabeled',
                    labeled_reacquired_images[0], iter_num)
                writer.add_image(
                    'sliceeq_saq/LabeledForegroundOccupancy',
                    labeled_occupancy[0, 1:2], iter_num)
                writer.add_image(
                    'sliceeq_saq/ReacquiredUnlabeled',
                    unlabeled_reacquired_images[0], iter_num)
                writer.add_image(
                    'sliceeq_saq/UnlabeledForegroundOccupancy',
                    unlabeled_occupancy[0, 1:2], iter_num)
                writer.add_image(
                    'sliceeq_saq/CenterTeacherMask',
                    pseudo_stack[0, center].unsqueeze(0) * 50, iter_num)

            if iter_num > 0 and iter_num % 200 == 0:
                logging.info(
                    'SliceEqSAQ train iteration %d: lambda=%.6f '
                ...
                logging.info(
                    'SliceEqSAQ occupancy iteration %d: '
                ...
                logging.info(
                    'SliceEqSAQ quadrature iteration %d: '
                    'coverage(L/U)=%.3f/%.3f; '
                    'max_count_deviation(L/U)=%.6f/%.6f',
                    iter_num,
                    diagnostics[
                        'labeled_quadrature_node_coverage'].item(),
                    diagnostics[
                        'unlabeled_quadrature_node_coverage'].item(),
                    diagnostics[
                        'labeled_quadrature_max_count_deviation'].item(),
                    diagnostics[
                        'unlabeled_quadrature_max_count_deviation'].item())
