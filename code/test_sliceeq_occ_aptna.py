"""Strict inference entry for independent SliceEqOccAPTNA checkpoints."""
...
parser.add_argument(
    '--save_result', '--save_results', dest='save_result', type=str,
    default='False',
    choices=['True', 'False', 'true', 'false', '1', '0', 'yes', 'no'],
    help='whether to save prediction nii.gz files; disabled by default')
...
def _strict_load_model_weight(evaluator, net, model_path, device):
    """Load only an exactly compatible SliceEqOccAPTNA state dictionary."""
    if not os.path.isfile(model_path):
        raise FileNotFoundError('Model file not found: {}'.format(model_path))
    checkpoint = torch.load(model_path, map_location=device)
    state_dict = evaluator.extract_state_dict(checkpoint, model_path)
    state_dict = evaluator.strip_module_prefix(state_dict)
...
if __name__ == '__main__':
    ...
    base.load_model_weight = lambda net, path, device: \
        _strict_load_model_weight(base, net, path, device)
