"""Scan-coherent fractional-occupancy SliceEq on the locked PROMISE12 recipe.

This independent successor changes only how SliceEqOcc samples the virtual
slice profile.  One continuous profile is shared by every slice of a patient
within an epoch, while randomized cross-patient stratification and epoch-wise
refresh preserve the original profile marginals and their tails.
"""
...
from dataloaders.sliceeq_dataset import (
    parse_slice_name, SliceStackDataSets, StackRandomGenerator)
...
from utils.sliceeq_scan import (
...
parser.add_argument('--exp', type=str, default='SliceEqOccSC_PROMISE12',
...
parser.add_argument(
    '--sliceeq_protocol_refresh_epochs', type=int, default=1,
    help='epochs for which one patient-level virtual profile remains fixed')
...
def _validate_args(flags):
    ...
    if flags.batch_size != 24 or flags.labeled_bs != 12:
        raise ValueError(
            'SliceEqOccSC final recipe expects effective student batch 36; '
            'keep loader batch_size=24 and labeled_bs=12')
    if flags.sliceeq_protocol_refresh_epochs < 1:
        raise ValueError(
            'sliceeq_protocol_refresh_epochs must be a positive integer')
...
def self_train(flags, pretrained_checkpoint, snapshot_path):
    ...
    sample_case_names = [
        parse_slice_name(sample_name)[0]
        for sample_name in db_train.sample_list
    ]
    labeled_protocol_cases = sorted(set(
        sample_case_names[:labeled_slice]))
    unlabeled_protocol_cases = sorted(set(
        sample_case_names[labeled_slice:]))
    if set(labeled_protocol_cases) & set(unlabeled_protocol_cases):
        raise ValueError(
            'SliceEqOccSC requires disjoint labeled/unlabeled patient sets')
    ...
    logging.info(
        'SliceEqOccSC profile: offsets=(-1,0,1), sigma=[%.3f, %.3f], '
        'phase=[%.3f, %.3f]',
        flags.sliceeq_sigma_min, flags.sliceeq_sigma_max,
        flags.sliceeq_phase_min, flags.sliceeq_phase_max)
    logging.info(
        'SliceEqOccSC effective student batch after warmup: 36 '
        '(12 original-L + 12 reacquired-L + 12 reacquired-U)')
    logging.info(
        'SliceEqOccSC objective: Lsup=0.5*(L_original_hard + '
        'L_reacquired_labeled_soft); total=Lsup+lambda*L_unlabeled_soft')
    labeled_protocol_seed = flags.seed + 100003
    unlabeled_protocol_seed = flags.seed + 200003
    logging.info(
        'SliceEqOccSC scan protocols: refresh_epochs=%d, labeled_cases=%d, '
        'unlabeled_cases=%d, labeled_seed=%d, unlabeled_seed=%d',
        flags.sliceeq_protocol_refresh_epochs,
        len(labeled_protocol_cases), len(unlabeled_protocol_cases),
        labeled_protocol_seed, unlabeled_protocol_seed)
    current_refresh_id = None
    labeled_protocol_table = None
    unlabeled_protocol_table = None
    offsets = tuple(range(
    ...
    for epoch_index in iterator:
        refresh_id = (
            epoch_index // flags.sliceeq_protocol_refresh_epochs)
        if refresh_id != current_refresh_id:
            labeled_protocol_table = build_scan_protocol_table(
                labeled_protocol_cases,
                (flags.sliceeq_sigma_min, flags.sliceeq_sigma_max),
                (flags.sliceeq_phase_min, flags.sliceeq_phase_max),
                refresh_id=refresh_id, base_seed=labeled_protocol_seed)
            unlabeled_protocol_table = build_scan_protocol_table(
                unlabeled_protocol_cases,
                (flags.sliceeq_sigma_min, flags.sliceeq_sigma_max),
                (flags.sliceeq_phase_min, flags.sliceeq_phase_max),
                refresh_id=refresh_id, base_seed=unlabeled_protocol_seed)
            current_refresh_id = refresh_id
            logging.info(
                'SliceEqOccSC protocol refresh %d at epoch %d',
                current_refresh_id, epoch_index)
        for _, sampled_batch in enumerate(trainloader):
            ...
            batch_case_names = [
                parse_slice_name(sample_name)[0]
                for sample_name in sampled_batch['case']
            ]
            labeled_case_names = batch_case_names[:flags.labeled_bs]
            unlabeled_case_names = batch_case_names[flags.labeled_bs:]
            ...
            if iter_num < 1000:
                ...
                diagnostics = {
                    ...
                    'unlabeled_center_foreground_fraction': (
                        center_pseudo > 0).float().mean(),
                    'labeled_scan_unique_case_fraction': zero,
                    'labeled_scan_within_case_sigma_range_max': zero,
                    'labeled_scan_within_case_phase_range_max': zero,
                    'unlabeled_scan_unique_case_fraction': zero,
                    'unlabeled_scan_within_case_sigma_range_max': zero,
                    'unlabeled_scan_within_case_phase_range_max': zero,
                }
            else:
                with torch.no_grad():
                    ...
                    unlabeled_weights, unlabeled_sigma, unlabeled_phase = \
                        scan_coherent_slice_profiles(
                            unlabeled_case_names, unlabeled_protocol_table,
                            offsets, device=volume_batch.device,
                            dtype=volume_batch.dtype)
                    labeled_weights, labeled_sigma, labeled_phase = \
                        scan_coherent_slice_profiles(
                            labeled_case_names, labeled_protocol_table,
                            offsets, device=volume_batch.device,
                            dtype=volume_batch.dtype)
                    ...
                    diagnostics.update(_prefixed(
                        'unlabeled_', occupancy_diagnostics(
                            ...
                            pseudo_stack[:, center])))
                    diagnostics.update(_prefixed(
                        'labeled_scan_', scan_coherence_diagnostics(
                            labeled_case_names, labeled_sigma,
                            labeled_phase)))
                    diagnostics.update(_prefixed(
                        'unlabeled_scan_', scan_coherence_diagnostics(
                            unlabeled_case_names, unlabeled_sigma,
                            unlabeled_phase)))
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError(
                    'Non-finite SliceEqOccSC loss at iteration {}'.format(
                        iter_num))
            loss.backward()
            optimizer.step()
            ...
            for name, value in diagnostics.items():
                if not torch.isfinite(value):
                    raise FloatingPointError(
                        'Non-finite SliceEqOccSC diagnostic {}'.format(name))
                writer.add_scalar('sliceeq_occ_sc/' + name, value, iter_num)
            writer.add_scalar(
                'sliceeq_occ_sc/labeled_neighbor_clamped_sample_fraction',
                labeled_clamped.mean(), iter_num)
            writer.add_scalar(
                'sliceeq_occ_sc/unlabeled_neighbor_clamped_sample_fraction',
            ...
            if iter_num % 20 == 0:
                ...
                writer.add_image(
                    'sliceeq_occ_sc/ReacquiredLabeled',
                    labeled_reacquired_images[0], iter_num)
                writer.add_image(
                    'sliceeq_occ_sc/LabeledForegroundOccupancy',
                    labeled_occupancy[0, 1:2], iter_num)
                writer.add_image(
                    'sliceeq_occ_sc/ReacquiredUnlabeled',
                    unlabeled_reacquired_images[0], iter_num)
                writer.add_image(
                    'sliceeq_occ_sc/UnlabeledForegroundOccupancy',
                    unlabeled_occupancy[0, 1:2], iter_num)
                writer.add_image(
                    'sliceeq_occ_sc/CenterTeacherMask',
                    pseudo_stack[0, center].unsqueeze(0) * 50, iter_num)

            if iter_num > 0 and iter_num % 200 == 0:
                logging.info(
                    'SliceEqOccSC train iteration %d: lambda=%.6f '
                ...
                logging.info(
                    'SliceEqOccSC occupancy iteration %d: '
                ...
                logging.info(
                    'SliceEqOccSC protocol iteration %d: refresh=%d; '
                    'unique_case_fraction(L/U)=%.4f/%.4f; '
                    'within_case_sigma_range(L/U)=%.8f/%.8f; '
                    'within_case_phase_range(L/U)=%.8f/%.8f',
                    iter_num, current_refresh_id,
                    diagnostics[
                        'labeled_scan_unique_case_fraction'].item(),
                    diagnostics[
                        'unlabeled_scan_unique_case_fraction'].item(),
                    diagnostics[
                        'labeled_scan_within_case_sigma_range_max'].item(),
                    diagnostics[
                        'unlabeled_scan_within_case_sigma_range_max'].item(),
                    diagnostics[
                        'labeled_scan_within_case_phase_range_max'].item(),
                    diagnostics[
                        'unlabeled_scan_within_case_phase_range_max'].item())
...
if __name__ == '__main__':
    ...
    logging.info(
        '================ START SliceEqOccSC SELF-TRAINING ================')
