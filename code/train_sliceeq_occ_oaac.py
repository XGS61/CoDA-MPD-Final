"""Ordered acquisition-appearance consistency on the locked SliceEqOcc parent.

Only the post-warmup unlabeled re-acquired student image receives the monotonic
coordinate-preserving appearance transform. Parent validation and inference are
...
from utils.sliceeq_oaac import ordered_appearance_transform
...
def _import_parent():
    ...
    try:
        ...
        import train_sliceeq_occ as parent
    finally:
...
parent.parser.set_defaults(exp='SliceEqOccOAAC_PROMISE12')
...
def _validate_args(flags):
    ...
    for name, expected in locked_recipe.items():
        ...
        if actual != expected:
            raise ValueError(
                'H7.13 locks --{}={!r}; received {!r}'.format(
...
if __name__ == '__main__':
    ...
    if parent_hash != EXPECTED_PARENT_TRAIN_SHA256:
        raise RuntimeError(
            'H7.13 parent train source changed: expected {}; got {}'.format(
    ...
    if checkpoint_hash != EXPECTED_PRETRAINED_SHA256:
        raise RuntimeError(
            'H7.13 requires shared Pre10000 SHA-256 {}; got {}'.format(
    ...
    logging.info(
        '================ START SliceEqOccOAAC SELF-TRAINING ================')
    logging.info(str(args))
    logging.info(
        'OAAC post-acquisition U appearance: log-gamma=[-0.20,0.20], '
        'log-contrast=[-0.15,0.15], brightness-span=[-0.10,0.10], seed=%d',
        APPEARANCE_SEED)
    logging.info(
