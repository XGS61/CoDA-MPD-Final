"""Slab-coherent pseudo occupancy on the locked SliceEqOcc recipe.

After the inherited 1k warmup, the only method change is to form one
26-connected foreground component over each three-slice unlabeled teacher slab
before applying the existing paired SliceEq profile. Validation and 2D
...
from utils.sliceeq_scpo import (
...
parser.add_argument('--exp', type=str, default='SliceEqOccSCPO_PROMISE12',
...
def _validate_args(flags):
    ...
    for name, expected in locked_recipe.items():
        ...
        if actual != expected:
            raise ValueError(
                'H7.12 locks --{}={!r}; received {!r}'.format(
...
def _validate_pretrained_checkpoint_hash(path):
    ...
    if actual != EXPECTED_PRETRAINED_SHA256:
        raise RuntimeError(
            'H7.12 requires shared Pre10000 checkpoint SHA-256 {}; got {} '
...
def self_train(flags, pretrained_checkpoint, snapshot_path):
    ...
    logging.info(
        'SliceEqOccSCPO U target: one fixed 26-connected component per '
        'three-slice hard teacher slab before the unchanged SliceEq profile')
    logging.info(
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num < 1000:
                ...
                diagnostics = {
                    ...
                    'unlabeled_center_foreground_fraction': (
                        center_pseudo > 0).float().mean(),
                    'scpo_changed_pixel_fraction': zero,
                    'scpo_removed_parent_foreground_fraction': zero,
                    'scpo_added_parent_foreground_fraction': zero,
                    'scpo_coherent_to_parent_foreground_mass_ratio':
                        volume_batch.new_tensor(1.0),
                    'scpo_raw_foreground_fraction': (
                        center_pseudo > 0).float().mean(),
                    'scpo_parent_2d_foreground_fraction': (
                        center_pseudo > 0).float().mean(),
                    'scpo_coherent_foreground_fraction': (
                        center_pseudo > 0).float().mean(),
                    'scpo_raw_component_count_mean': zero,
                    'scpo_retained_component_count_mean': zero,
                    'scpo_active_slab_fraction': zero,
                }
            else:
                with torch.no_grad():
                    ...
                    raw_pseudo_stack = base.get_masks(
                        ema_output, nms=0).reshape(
                            unlabeled_count, stack_size, height, width)
                    parent_2d_pseudo_stack = base.get_2DLargestCC(
                        raw_pseudo_stack.reshape(
                            unlabeled_count * stack_size,
                            height, width)).reshape(
                                unlabeled_count, stack_size, height, width)
                    pseudo_stack, raw_component_counts = \
                        slab_largest_connected_component(raw_pseudo_stack)
                    ...
                    diagnostics.update(_prefixed(
                        'unlabeled_', occupancy_diagnostics(
                            ...
                            pseudo_stack[:, center])))
                    diagnostics.update(_prefixed(
                        'scpo_', scpo_diagnostics(
                            raw_pseudo_stack, parent_2d_pseudo_stack,
                            pseudo_stack, raw_component_counts)))
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                logging.info(
                    'SliceEqOccSCPO topology iteration %d: '
                    'changed/removed/added=%.6f/%.6f/%.6f; '
                    'mass_ratio=%.6f; components(raw/kept)=%.4f/%.4f; '
                    'active_slabs=%.6f',
                    iter_num,
                    diagnostics['scpo_changed_pixel_fraction'].item(),
                    diagnostics[
                        'scpo_removed_parent_foreground_fraction'].item(),
                    diagnostics[
                        'scpo_added_parent_foreground_fraction'].item(),
                    diagnostics[
                        'scpo_coherent_to_parent_foreground_mass_ratio'].item(),
                    diagnostics['scpo_raw_component_count_mean'].item(),
                    diagnostics[
                        'scpo_retained_component_count_mean'].item(),
                    diagnostics['scpo_active_slab_fraction'].item())
...
if __name__ == '__main__':
    ...
    logging.info(
        '=============== START SliceEqOccSCPO SELF-TRAINING ===============')
